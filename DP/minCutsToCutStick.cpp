...
// When you cut a stick, it will be split into two smaller sticks (i.e. the sum of their lengths
...
// The first cut is done to a rod of length 7 so the cost is 7. The second cut is done to a
// rod of length 6 (i.e. the second part of the first cut), the third is done to a rod of length 4 and the
...
public:
    ...
    int f(int start, int end, vector<int> &cuts, vector<vector<int>> &dp)
    {
        ...
        {
            ans = min(ans, cuts[end + 1] - cuts[start - 1] + f(start, k - 1, cuts, dp) + f(k + 1, end, cuts, dp));
        }
